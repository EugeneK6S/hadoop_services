        ...
        if ( n['hadoop_services']['ssh']['public_key'] != nil ) then
				...
				if (node['hadoop_services'].has_key? 'is_active_nn' and node['hadoop_services']['is_active_nn'] == "true")
					...
					ruby_block "report_namenode_status" do
					    block do
							...
							node.set['hadoop_services']['already_active_namenode'] = true
					    end
					    action :nothing
					end
			...
			##################################################################################################################
					...
					search(:node, "project:#{node['project']} AND role:hadoop-namenode AND hadoop_services_is_active_nn:true") do |m|
						if ( m['hadoop_services']['already_active_namenode'] ) then
							...
							rewind "execute[hdfs-namenode-format]" do
							    ...
							    group "root"
							  	user "root"
							    notifies :run, 'execute[hdfs-chown-dirs]', :immediately
							    notifies :run, 'execute[zookeeper-server-start]', :immediately
							end
